...
class quantileRegression_chain(object):
    ...
    def trainOnData(self,var,num_hidden_layers,num_units,act,weightsDir,clusterconfig=None):
        """
        Method to train quantile regression BDTs on data. See ``_trainQuantiles`` for Arguments
        """

        self._trainQuantiles('data',var,num_hidden_layers,num_units,act,weightsDir,clusterconfig=clusterconfig)

    def trainOnMC(self,var,num_hidden_layers,num_units,act,weightsDir,clusterconfig=None):
        """
        Method to train quantile regression BDTs on MC. See ``_trainQuantiles`` for Arguments
        """

        self._trainQuantiles('mc',var,num_hidden_layers,num_units,act,weightsDir,clusterconfig=clusterconfig)

    def gen_scale_par(self, weightsDir, scale_file):
        """
        Method to generate the mean and std values to scale features and target for both training and test
        --------
        scale_file: string, the name of the file to save the scale parameters
        """
        weightsDir = weightsDir if weightsDir.startswith('/') else '{}/{}'.format(self.workDir, weightsDir)
        self.scale_par = '{}/{}'.format(weightsDir, scale_file)

        par = pd.DataFrame([self.data.mean(), self.data.std()], index=['mu', 'sigma'])
        par.to_hdf(self.scale_par, key='scale_par', mode='w')


    def _trainQuantiles(self,key,var,num_hidden_layers,num_units,act,weightsDir,clusterconfig=None):
        """
        ...
        cluster, client = self.setup_cluster(clusterconfig)
#        cluster.scale(21)
#        client.wait_for_workers(1)
        ...
        futures = [client.submit(trainQuantile,
                                 ...
                                 act,
                                 batch_size = 8192,
                                 scale_par = self.scale_par,
                                 outDir = weightsDir if weightsDir.startswith('/') else '{}/{}'.format(self.workDir,weightsDir),
                                 file_name = '{}_models_{}_{}_{}'.format(name_key,self.EBEE,var,str(quantile).replace('.','p'))
...
#        client.wait_for_workers(1)
    ...
    def trainAllMC(self,num_hidden_layers,num_units,act,weightsDir,clusterconfig=None):
        """
...
#                self.loadClfs(var,weightsDir)
                self.loadModels(var,weightsDir)
            except IOError:
                self.trainOnMC(var,num_hidden_layers,num_units,act,weightsDir,clusterconfig)
...
#                self.loadClfs(var,weightsDir)
            ...
            self.correctY(var,clusterconfig=clusterconfig)

    def loadModels(self,var,weightsDir):
        """
        Method to load tensorflow models
        """
        weightsDir = weightsDir if weightsDir.startswith('/') else '{}/{}'.format(self.workDir, weightsDir)
        self.models_mc = ['mc_models_{}_{}_{}'.format(self.EBEE,var,str(q).replace('.','p')) for q in self.quantiles]
        self.models_d = ['data_models_{}_{}_{}'.format(self.EBEE,var,str(q).replace('.','p')) for q in self.quantiles]
    ...
    def setup_cluster(self, config_file = None):
        ...
        logger.info('Setting up Client {}'.format(client))
        return cluster, client

    def close_cluster(self, cluster, client):
        client.close()
        cluster.close()
