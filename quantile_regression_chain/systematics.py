...
try:
    ...
    print('ROOT is not available in this environment. Syst file cannot be produced')
...
class utils(object):
    ...
    def clf_reweight(df_mc,df_data,n_jobs=1,cut=None):
        ...
        clf.fit(X,y.ravel())
        eps = 1.e-3
        arr = clf.predict_proba(df_mc.loc[:,features].values)
        ret = np.apply_along_axis(lambda x: x[1]/(x[0]+eps), 1, arr)
        return ret
...
class systematics(object):
    ...
    def saveSystFile(self, ofile, df_data):
        ...
        if 'ROOT' in sys.modules:
            ...
            f = ROOT.TFile(ofile,'UPDATE')
            gr_up = ROOT.TGraph(len(x),x,y_up)
            gr_down = ROOT.TGraph(len(x),x,y_down)

            gr_down.SetNameTitle(title+'down',title+'down')
            gr_up.SetNameTitle(title+'up',title+'up')

            gr_up.Write(title+'up',ROOT.gSystem.kOverwrite)
            gr_down.Write(title+'down',ROOT.gSystem.kOverwrite)

            f.Close()

        else:
            print('ROOT not available in this environment. File will be saved as pytables dataframe')
            if '.root' in ofile:
                ofile = ofile.replace('.root','.h5')
