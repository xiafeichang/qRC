...
class Corrector:
   ...
   # store regressors
   def __init__(self,mc_models,data_models,scale_par,target,X,Y,diz=False):
      self.diz=diz #Flag for distribution with discrete 0, i.e. Isolation
      self.mcqtls   = np.array([predict(X,model,scale_par,target) for model in mc_models])
      self.dataqtls = np.array([predict(X,model,scale_par,target) for model in data_models])
