...
class quantileRegression_chain_disc(quantileRegression_chain):
    ...
    def trainp0tclf(self,var,key,weightsDir ='weights_qRC',n_jobs=1):
        ...
        if self.client:
            n_jobs = len(self.client.scheduler_info()['workers'])
        xgb_clf_args = {
    ...
    def trainTailRegressors(self,var,weightsDir='weights_qRC'):
        ...
        if self.client:
            logger.info('Training tail regressor for variable {}'.format(var))
    ...
    def shiftY(self,var,finalReg=False,n_jobs=1):
        ...
        if self.client:
            logger.info('Shifting Y for variable {}'.format(var))
    ...
    def shiftY2D(self,varrs,finalReg=False,n_jobs=1):
        ...
        if self.client:
            logger.info('Shifting Y for variables {}'.format(varrs))
            n_workers = len(self.client.scheduler_info()['workers']) if len(self.client.scheduler_info()['workers']) > 0 else len(self.quantiles)
            if finalReg:
