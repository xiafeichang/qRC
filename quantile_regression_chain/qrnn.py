...
def trainQuantile(X, Y, q, num_hidden_layers=1, num_units=None, act=None, batch_size=64, scale_par=None, outDir=None, file_name=None):

    save_file = '{}/{}'.format(outDir, file_name)
...
def scale(df, scale_file):
    ...
    for var in df.keys(): 
        ...
        vars_raw.append(var_raw)

    par = pd.read_hdf(scale_file).loc[:,vars_raw] 
#    print(par)
