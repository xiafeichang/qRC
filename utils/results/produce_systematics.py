...
def plotOrig(systs, df_data, saveDir, cut=None, label=None, zoom=False):
    ...
    plltt = pldmc.plot_dmc_hist(systs.df, df_data, ratio=True, norm=True, cut_str='', label=label, **dic)
    plltt.draw()
...
def main(options):
    ...
    # In MC
    # N.B.: probePhoIdMVA = newPhoIDcorrAllFinal
    # N.B.: probePhoIdMVA_uncorr = newPhoID
    # In data there is no correction performed in newPhoIDcorrAllFinal
    if 'probePhoIdMVAtr' not in df_in.columns:
        df_in['probePhoIdMVAtr'] = syst.utils.get_quantile(df_in,df_data,'newPhoIDcorrAllFinal','newPhoID',weights='weight')
    ...
    df_data['probePhoIdMVAtr'] = syst.utils.get_quantile(df_data,df_data,'newPhoID','newPhoID',weights='weight')
